...
db.serialize(() => {
    ...
    // Add flowRate column if it doesn't exist
    db.all("PRAGMA table_info(tank_history)", [], (err, columns) => {
    if (err) {
        console.error('Error checking tank_history schema:', err);
        return;
    }
    
    const hasFlowRate = columns && columns.some(col => col.name === 'flowRate');
    const hasFlowStatus = columns && columns.some(col => col.name === 'flowStatus');
    
    if (!hasFlowRate) {
        console.log('⚙️ Adding flowRate column to tank_history...');
        db.run(`ALTER TABLE tank_history ADD COLUMN flowRate REAL DEFAULT 0`, (err) => {
            if (err && !err.message.includes('duplicate column')) {
                console.error('Error adding flowRate column:', err);
            } else {
                console.log('✅ Added flowRate column to tank_history');
            }
        });
    }
    
    if (!hasFlowStatus) {
        console.log('⚙️ Adding flowStatus column to tank_history...');
        db.run(`ALTER TABLE tank_history ADD COLUMN flowStatus TEXT DEFAULT 'stable'`, (err) => {
            if (err && !err.message.includes('duplicate column')) {
                console.error('Error adding flowStatus column:', err);
            } else {
                console.log('✅ Added flowStatus column to tank_history');
            }
        });
    }
});
    ...
    db.run(`
        CREATE TABLE IF NOT EXISTS tank_history (
            ...
            flowRate REAL DEFAULT 0,
            flowStatus TEXT DEFAULT 'stable',
...
function calculateSupplyOverview(tanks, valves, pipelines) {
    console.log('📈 Calculating supply overview (simplified)...');
    
    const mainValves = valves.filter(v => v.category === 'main');
    const subValves = valves.filter(v => v.category === 'sub');
    
    // Calculate basic stats without flow calculation
    let totalHouseholds = 0;
    let servedHouseholds = 0;
    let totalFlow = 0;
    
    // Count households from all valves
    valves.forEach(valve => {
        totalHouseholds += valve.households || 0;
        
        // If valve is open, count its households as served
        if (valve.isOpen) {
            servedHouseholds += valve.households || 0;
            totalFlow += valve.flowRate || 0;
        }
    });
    ...
    // Build regions by mandal
    const regions = {};
    valves.forEach(valve => {
        const mandalName = valve.mandal || 'Unknown';
        
        if (!regions[mandalName]) {
            regions[mandalName] = {
                name: mandalName,
                valves: [],
                totalHouseholds: 0,
        ...
        regions[mandalName].valves.push({
            valveId: valve.valveId,
            name: valve.name,
            households: valve.households || 0,
            flow: valve.isOpen ? (valve.flowRate || 0) : 0,
            isOpen: valve.isOpen,
            habitation: valve.habitation || ''
        });
        
        regions[mandalName].totalHouseholds += valve.households || 0;
        
        if (valve.isOpen) {
            regions[mandalName].servedHouseholds += valve.households || 0;
            regions[mandalName].totalFlow += valve.flowRate || 0;
        }
    });
    
    console.log(`✅ Supply overview: ${servedHouseholds}/${totalHouseholds} households (${coverage.toFixed(1)}%)`);
    ...
    onValue(deviceRef, (snapshot) => {
        ...
        if (data && data.distance !== undefined) {
            ...
            if (calculatedData) {
                ...
                db.run(
                    `INSERT INTO tank_history (deviceId, tankId, name, distance, waterLevel, waterLevelCm, volume, percentage, pressure, weight, flowRate, flowStatus, status, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    ...
                    enrichedData.pressure, enrichedData.weight, enrichedData.flowRate, enrichedData.flowStatus, enrichedData.status, enrichedData.timestamp],
                    function(err) {
                        if (err) {
                            console.error(`❌ Error storing history for ${deviceId}:`, err.message);
                        } else {
                            console.log(`📜 ${deviceId} history entry #${this.lastID} created with flow rate`);
                        }
                    }
                );
...
// 🔐 DEVICE TOKEN (simple auth)
const DEVICE_TOKEN = "JMS_DEVICE_001";

// 🔥 FIREBASE DETAILS (PASTE HERE)
const FIREBASE_DB_URL =
  "https://jal-mahakal-shakti-default-rtdb.asia-southeast1.firebasedatabase.app";

const FIREBASE_DB_SECRET =
  "SItfk9tDQSOfISqZCUHRzJpZeQH7q2GtGnxk1OmE";
...
app.post("/pushData", async (req, res) => {
  try {
    ...
    const { deviceId, pressure, flow, motor } = req.body;
    ...
    // 🔗 Firebase REST URL
    const url = "https://jal-mahakal-shakti-default-rtdb.asia-southeast1.firebasedatabase.app/pipeline/node01.json?auth=SItfk9tDQSOfISqZCUHRzJpZeQH7q2GtGnxk1OmE";
    ...
    if (!fbRes.ok) {
      ...
      return res.status(500).json({ error: "Firebase error", details: txt });
    }

    console.log("Data pushed:", deviceId, payload);
...
app.listen(3000, () => {
  console.log("🔥 Node.js Firebase REST API running on port 3000");
});
// ==================== CLEANUP ON EXIT ====================
